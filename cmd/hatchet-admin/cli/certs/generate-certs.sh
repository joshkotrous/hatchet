...
if [ -z "$1" ]; then
  echo "Error: Certificate directory not provided"
...
if [[ "$CERTS_DIR" == *"../"* || "$CERTS_DIR" == *"/.."* || "$CERTS_DIR" == ".." || "$CERTS_DIR" == "../"* ]]; then
  echo "Error: Directory path contains path traversal patterns"
  exit 1
fi

# Check if the directory exists, if not create it
if [ ! -d "$CERTS_DIR" ]; then
  mkdir -p "$CERTS_DIR" || { echo "Error: Failed to create directory $CERTS_DIR"; exit 1; }
fi

# Check if the directory is writable
if [ ! -w "$CERTS_DIR" ]; then
  echo "Error: Directory $CERTS_DIR is not writable"
  exit 1
fi

echo "generating certs in dir: $CERTS_DIR"
